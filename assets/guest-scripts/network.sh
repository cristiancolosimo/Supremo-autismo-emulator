...
if (${FIRMAE_NET}); then
  ...
  if [ ${ACTION} == "default" ]; then
    # tenta il bridge; se brctl non è disponibile (applet non compilata) ripiega
    # sull'IP diretto su eth0 — sufficiente per la raggiungibilità (slirp/TAP).
    if ${BUSYBOX} brctl addbr br0 2>/dev/null; then
      ${BUSYBOX} ifconfig br0 192.168.0.1
      ${BUSYBOX} brctl addif br0 eth0
      ${BUSYBOX} ifconfig eth0 0.0.0.0 up
    else
      ${BUSYBOX} ifconfig eth0 192.168.0.1 up
    fi
